...
class MongooseConnectionManager {
  ...
  public addConnection = async (
    ...
    if (this.Connections.has(name)) {
      console.log(
        `\x1b[33m[WARNING]\x1b[0m Connection "${name}" already exists.`
      );
      return;
    }
    ...
    try {
      ...
      conn.on("connected", () => {
        console.log(
          `\x1b[32m[INFO]\x1b[0m Connection "${name}" established successfully.`
        );
      });

      conn.on("error", (err: Error) => {
        console.log(
          `\x1b[31m[ERROR]\x1b[0m Connection "${name}" error: ${err.message}`
        );
      });
      ...
      console.log(
        `\x1b[31m[ERROR]\x1b[0m Failed to create connection "${name}": ${error.message}`
      );
    }
  };
  ...
  public getAllConnections(
    withDetails: true
  ): { name: string; state: number; models: string[]; uri: string }[];
  public getAllConnections(
  ...
  public closeConnection = async (name: string): Promise<void> => {
    ...
    if (connection) {
      ...
      console.log(
        `\x1b[32m[INFO]\x1b[0m Connection "${name}" closed successfully.`
      );
    } else {
      console.log(
        `\x1b[33m[WARNING]\x1b[0m Attempted to close non-existent connection "${name}".`
      );
    }
  };
    ...
    for (const [name, conn] of this.Connections.entries()) {
      connectionPromises.push(
        conn
          ...
          .catch((error: any) =>
            console.error(
              `\x1b[31m[ERROR]\x1b[0m Error closing connection "${name}": ${error.message}`
            )
          )
      );
    }
